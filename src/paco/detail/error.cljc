...
(defn nested
  ...
  (if (message? error ::nested)
    error
    (->Nested ::nested nil (scanner/position scanner) error)))
...
(defn compound
  ...
  (if (message? error ::nested)
    ...
    (->Nested ::compound (str label) (scanner/position scanner) error)))
...
(defn unexpected-token-or-end
  ([scanner]
   (if (scanner/end? scanner)
     unexpected-end
     (unexpected-input (scanner/peek scanner))))
  ([scanner error]
   (merge error (unexpected-token-or-end scanner))))
